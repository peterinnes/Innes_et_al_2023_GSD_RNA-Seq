...
a <- read.csv("data2/seedling_data/GSDgreenhouseExpt2018_Data_sheet1.csv",
              header = T) %>%
...
b <- read.csv("data2/seedling_data/GSDgreenhouseExpt2018_Data_sheet2.csv",
              header = T) %>%
...
fit_leaf_mass_proportion <- lm(LEAF_MASS_DRY/PLANT_MASS ~ -1 + HABITAT, data = phenotype_data)
