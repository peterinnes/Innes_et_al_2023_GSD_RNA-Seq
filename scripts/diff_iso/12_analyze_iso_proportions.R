...
# 2023-02-01/good_isoforms.post_consolidate_filtered.2023-02-05.txt
spliced_genes <- read.table("analysis/diff_iso_out/2023-07-03/good_isoforms.post_consolidate_filtered.2023-07-03.txt", col.names = c("gene","isoforms")) %>%
...
#2023-02-01/consolidated_isoform_clusters.with_rownames.2023-02-05.txt
isoform_clusters <- read.table("analysis/diff_iso_out/2023-07-03/consolidated_isoform_clusters.with_rownames.2023-07-03.txt",
...
isoform_tpm_df_clusters <- left_join(isoform_clusters, isoform_tpm_df) %>%
  group_by(cluster, gene) %>%
  summarise_if(is.numeric, sum) %>%
  relocate(gene,cluster) %>%
  dplyr::rename(isoform_id=cluster) #rename to match the no_clusters df
...
isoform_tpm_df_no_clusters <- anti_join(isoform_tpm_df, isoform_clusters) %>%
  inner_join(spliced_genes) %>%
  relocate(gene, isoform_id)

# combine the clusters and no_clusters isoform tpm dataframes into our final tpm df
diff_iso_tpm_df <- data.frame(rbind(isoform_tpm_df_clusters,
                                    isoform_tpm_df_no_clusters)) %>%
  arrange(gene)
...
# 2023-02-01/genes_with_two_isoforms.txt
Trinity_genes_with_two_isos <- read.table("analysis/diff_iso_out/2023-07-03/genes_with_two_isoforms.txt",
                                          col.names = "gene")
