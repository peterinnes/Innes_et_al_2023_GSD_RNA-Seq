...
gene_counts_raw <- read.table("data2/RSEM_out_2023-02-01/gene_counts_matrix.txt",
                               header = T, row.names = 1)

isoform_counts_raw <- read.table("data2/RSEM_out_2023-02-01/isoform_counts_matrix.txt",
                                 header=T, row.names = 1)
...
low_expr <- left_join(low_expressed_genes, iso_gene_map) %>%
  ...
  full_join(low_expressed_isoforms)
...
# filter the originial raw counts matrix
isoform_counts_filtered <- isoform_counts_raw[!rownames(isoform_counts_raw) %in%
                                                low_expr$isoform,]
